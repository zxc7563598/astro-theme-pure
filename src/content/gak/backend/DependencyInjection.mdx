---
title: '设计说明: 依赖注入'
description: '本文介绍 GoAdminKit 项目中的依赖注入设计与实现方式，通过集中式的初始化流程，将 repository、service、handler 等各层依赖进行统一管理与装配。相比零散注入，这种方式能够更清晰地控制模块边界，降低耦合度，同时提升代码的可维护性与可扩展性。'
order: 2
---

在一个分层明确的项目中，**依赖注入**  是非常关键的一环。

如果依赖关系在各个模块中随意创建，很容易导致代码耦合混乱，后期维护成本会迅速上升。因此在 GoAdminKit 中，我选择将所有依赖注入逻辑​**集中在** **`internal/bootstrap`** **目录中统一管理**。
