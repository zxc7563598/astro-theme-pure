---
title: '设计说明: Model'
description: '本文介绍 GoAdminKit 项目中 Model 层的设计与使用规范，主要用于定义 GORM 映射结构体以及 repository 查询结构。通过统一的数据模型约束，明确 Model 层职责边界，避免业务逻辑侵入，从而保证数据层的简洁性与可维护性。'
order: 3
---


在 GoAdminKit 的分层架构中，Model 层位于最底层，负责定义数据库表结构在 Go 代码中的映射。它仅用于数据结构表达，不参与任何业务逻辑处理。
